...
let locations = [
  {
    title: "Muslim Concentration Camps",
    lat: 34.128627,
    lng: 105.766435,
    description:
  ...
  {
    title: "Humanitarian Crisis-Political and Economic Turmoil",
    lat: 7.02953,
    lng: -66.254809,
    description:
  ...
  {
    title: "Displacement and Food Insecurity",
    lat: 7.519221,
    lng: 29.999696,
    description:
...
function initMap() {
  map = new google.maps.Map(document.getElementById("map"), {
    ...
    zoom: 3,
    center: { lat: 28.024, lng: 20.2433205 },
...
function initMarkerClusterer() {
  ...
  markers = locations.map(function (location, i) {
    ...
    marker.addListener("click", () => {
      ...
      contentString =
        ...
        '<div id="site-notice">' +
        "</div>" +
        '<h1 id="first-heading" class="first-heading">' +
        marker.getTitle() +
        "</h1>" +
        '<div id="body-content">' +
        description +
  ...
  google.maps.event.addListener(markerClusterer, "click", function (c) {
    ...
    for (var i = 0; i < m.length; i++) {
      var markerPos =
        m[i].getPosition().lat() + ", " + m[i].getPosition().lng();
      p.push(
        '<div onclick="panToPosition(' +
          markerPos +
...
function confirmMarker() {
  ...
  if (title.length <= 0) title = "<i>Lorem Ipsum</i>";

  if (
    description === "Usage: <br> for line break (use HTML syntax!)" ||
  ...
  var audio = new Audio(
    "https://github.com/utsavneutron/marks_for_justice/blob/master/marker_place.mp3"
  );
